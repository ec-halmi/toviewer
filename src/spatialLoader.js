    ...
    for (const site of sites) {
      ...
      for (const b of this.jsonData[site][0]) {
        ...
        for (const s of this.jsonData[b][0]) {
          ...
          icon.style.color = "var(--bs-gray-400)";
          icon.innerHTML = "stacks";
    ...
    Object.entries(children).forEach(([type, elems]) => {
      if (elems.length > 0) {
        ...
        elems.forEach(async elem => {
          let byElem;
          if (elem[2].length === 0) { // if no child, elem[2].length = 0, data = null
            byElem = this._createAccordianItem(elemsAccordian.id, elem[1], null, elem[0]);
          } else { // hav child, neeed to create child accordian
            // create accordian
