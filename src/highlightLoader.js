...
export class HighlightLoader
{
  ...
  async hideHighlightedItem ( event, data )
  {
    ...
    if ( event.key === 'Escape' || event.key === 'Esc' )
    {
      ...
      // handles ifcspace
      const visibilityLoader = new VisibilityLoader( this.components, this.world, this.world.model );
      const lists = await visibilityLoader.classifierByStorey();
      this.hideHandler( lists.entities[ "IFCSPACE" ].map, false ); // hide IfcSpace
